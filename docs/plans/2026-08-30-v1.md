...
## Task 5: The health check

Fact 4 says that `claude auth status` lies for this purpose. So the health check
is one small real call.
...
def test_health_is_true_on_a_good_run(monkeypatch, tmp_path):
    monkeypatch.setenv("FAKE_MODE", "ok")
    ok, why = runner.health(binary=str(FAKE), cwd=tmp_path)
    assert ok is True


def test_health_is_false_when_authentication_fails(monkeypatch, tmp_path):
    monkeypatch.setenv("FAKE_MODE", "auth")
    ok, why = runner.health(binary=str(FAKE), cwd=tmp_path)
    assert ok is False
    assert why
...
Run: `.venv/bin/pytest tests/test_runner.py -q`
Expected: FAIL with `AttributeError: module 'nightshift.runner' has no attribute 'health'`.

- [ ] **Step 3: Add the function**

```python
# add to nightshift/runner.py
def health(*, cwd: pathlib.Path, binary: str = "claude") -> tuple[bool, str]:
    """One small real call. `claude auth status` is not a health check."""
    r = run("Answer with the word READY and nothing else.", cwd=cwd,
            binary=binary, timeout=120)
...
```bash
git add nightshift/runner.py tests/test_runner.py
git commit -m "Add a real health check"
```
...
def test_a_message_that_needs_an_answer_becomes_a_needs_you_item():
    ...
    items = mail.triage(fake, cwd="/tmp")
    assert len(items) == 1
    assert items[0].bucket == "needs_you"
    assert items[0].source_url.startswith("https://mail.google.com")
...
def test_a_message_with_no_action_goes_to_the_folded_list():
    ...
    items = mail.triage(fake, cwd="/tmp")
    assert items[0].bucket == "no_action"
...
def test_an_item_with_no_source_is_dropped():
    fake = FakeRunner({"messages": [
        {"id": "m3", "title": "x", "why": "y", "needs_action": True,
         "source_url": ""}]})
    assert mail.triage(fake, cwd="/tmp") == []
```
...
def triage(runner_module, cwd) -> list[Item]:
...
def write_draft(runner_module, item: "Item", cwd) -> str | None:
...
class Stub:
    def __init__(self, healthy=True, items=None):
        self.healthy, self.items = healthy, items or []
        self.ran, self.drafted = False, 0

    def health(self, **kw):
        return (self.healthy, "" if self.healthy else "401")
    ...
    def write_draft(self, *a, **kw):
        self.drafted += 1
        return "one line about the draft"


def test_a_bad_health_check_writes_a_failed_run_and_stops(tmp_path):
    conn = db.connect(tmp_path / "s.db")
    stub = Stub(healthy=False)
...
def run_once(conn: sqlite3.Connection, *, runner_module, mail_module,
    ...
    healthy, why = runner_module.health(cwd=workspace)
    if not healthy:
        _end_run(conn, run_id, False, error=why)
        return
    ...
    items = mail_module.triage(runner_module, cwd=workspace)
    for item in items:
        if item.bucket == "needs_you":
            mail_module.write_draft(runner_module, item, cwd=workspace)
        conn.execute(
...
CEILING_USD = float(os.environ.get("NIGHTSHIFT_CEILING_USD", "5.0"))
...
Four runs each day, as the spec says: 06:00, 11:00, 16:00, 21:00.
...
- launchd runs the cycle four times each day.
